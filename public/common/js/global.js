...
async function loadToolboxCategory(product) {
    ...
    try {
        ...
        document.getElementById('block-category-div').innerHTML = resultData;

        // TODO:
        // $(".menu-tab-item").removeClass("menu-tab-selected");

        document.getElementById('js--tab-' + product).classList.add('menu-tab-selected');
...
async function loadToolboxBlocks(product, defaultCategory) {
    ...
    try {
        const response = await fetch(url);
        const resultData = await response.text();
        document.getElementById('blockly-block-data').innerHTML = resultData;
...
var resizeBlockEditor = function () {
    ...
    document.getElementById('block-category-div').style.height = blockDivHeight;
...
var changeToolbox = function (name, category) {
    ...
    const blockCategoryDiv = document.getElementById('block-category-div');
    const blockCategoryBtns = blockCategoryDiv.getElementsByClassName('btn');
    for(let i=0; i < blockCategoryBtns.length; i++)  {
        blockCategoryBtns[i].classList.remove('category-selected');
    }    

    const categories = blockCategoryDiv.getElementsByClassName('.js--category-' + category + '-' + name);
...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    blockCategoryDiv = document.getElementById('block-category-div');

    ...
    <nav class="header-nav">
        ...
        <div class="menulist">
            <ul class="ui-left">
                ...
                <!-- <li class="ui-left-bt" @click="audioWrite('audioTemp0921','src/assets/sound/wait.wav')"> -->
...
export default {
    ...
    mounted() {
        ...
        window.addEventListener(
            "message",
            (event) => {
                // 유효성 검사: event.data가 문자열인지 확인
                if (typeof event.data === 'string') {
                    try {
                        serial.runCode(event.data);
                    } catch (error) {
                        console.error('runCode 실행 중 에러 발생:', error);
                    }
                } else {
                    console.warn('유효하지 않은 데이터 형식:', event.data);
                }
            },
            false,
        );
